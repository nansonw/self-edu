...
#### 浏览器的进程
```
  浏览器的内核通常是指的渲染引擎( > 渲染进程 ) + js引擎(v8)
   1 浏览器的进程
     1.1 Browser进程:浏览器的主进程,唯一,负责协调和主控.[浏览器级别的进程]
        :协调和主控:负责浏览器界面显示,用户交互,页面管理,进程管理,网络资源管理等 
     1.2 第三方插件进程:一个激活的插件===一个进程.[页面级别的进程]
     1.3 GPU进程:浏览器的唯一进程,用于3D绘制等.[浏览器级别的进程]
     1.4 渲染进程[重点理解]:(浏览器的内核,渲染线程只是其中一个线程) 这个进程是多线程的,负责页面渲染(线程甲)js执行(线程乙).[页面级别的进程]
  ...
  3 多线程的渲染进程(页面的渲染,js的执行,事件循环 浏览器的内核)
    3.1 GUI渲染线程:
    ...
    3.2 JS引擎线程
      ...
      iii)执行优先高于GUI渲染线程
      ---------------
    3.3 事件触发线程(分发事件,操作和集中回调到任务队列)
      i)控制事件循环,管理任务队列.不属于js引擎线程
      ii)分发事件或操作到相应的具体处理线程(定时触发线程/异步请求线程),得到结果后再将对应的回调操作添加到任务队列的末尾.
