...
#虽然绩效指数也能判断出进度和成本方面的偏差，但是它的主要实用场景是用来做预测的。
[成本预测]：

1 一条路走到黑（一直错。典型成本预测）：EAC(完工估价) = BAC（完工预算）/CPI   默认的主要考察方式是典型预测成本的方式。
2 会自救(前期犯错，后面会恢复正常。非典型成本预测): EAC = ETC(BAC - EV) + AC

[进度预测]
EAS(进度评估) = T(工期)/ SPI(进度绩效指数)

[完工尚需绩效指数]：事钱比。大于1不利
![完工尚需绩效指标](image-107.png)
剩下的事 = BAC - EV
剩下的钱 = BAC - AC
...
![挣值计算公式一：汇总](image-109.png)
![挣值计算公式二：汇总](image-110.png)
![练习](image-111.png)

正常项目结束的节点：BAC = PV = EV
EV确认规则，比较保守的是0到100，风险系数高的是50%，50%，风险系数较低的是20%，80%

![挣值管理：s曲线or香蕉图](image-112.png)
...
检查表：用数据找出缺陷数量，频次。
测试评估：测试质量结果
控制图：控制过程，设置上下限。判断是否失控。超过上下限都是失控，在均值一侧连续七点也算失控。
![控制图](image-114.png)
...
## 控制资源
考点：分配好的资源发生了变更需要走变更流程。

## 监督沟通
考点：找到正确的人用正确有效的方法正确的时间沟通准确的事情。

## 监督风险
![监督风险](image-115.png)
考点：技术与工具
监督风险：监督风险的应对实施、跟踪已识别的风险、识别和分析新风险、评估风险管理有效性分析。

技术绩效分析：产品、技术风险分析
储备分析：储备金与风险量是否匹配。
审计：审过程、查结果。风险管理过程的有效性，查过程。
会议：风险应对计划的有效性。查结果


## 控制采购
控制采购：管理采购关系、监督采购合同绩效、实施必要的合同变更和纠偏、关闭合同
考点：与活动相关的工具与技术
注意事项：
三大审计：质量、风险、采购。都是过程性的工作。
质量审计： 程序文件的执行过程。
风险审计：风险管理过程的有效性
采购审计：采购过程的经验教训。
发生采购合同纠纷的处理流程：谈判-》仲裁-》诉讼。 依次升级。
![工具与技术与活动对应关系](image-116.png)

## 监督干系人参与
管理干系人关系，引导干系人合理参与项目，适时解除干系人关系，