...
![基于Etag头实现的协商缓存](image-18.png)
![基于Etag头实现的协商缓存缺点](image-19.png)
```
 2) http缓存
   强制缓存：服务器通过设置响应头（Expires/Cache-control），告诉浏览器在指定时间内，可以缓存该资源，如果在指定时间内，客户端再次访问该资源，则直接从缓存中读取，而无需再次向服务器发起请求。
   ...
   小结:注意，我们的缓存主要是针对html,css,img等静态资源，常规情况下，我们不会去缓存一些动态资源，因为缓存动态资源的话，数据的实时性就不会不太好，所以我们一般都只会去缓存一些不太容易被改变的静态资源。有哈希值的文件设置强缓存即可。没有哈希值的文件（比如index.html）设置协商缓存
