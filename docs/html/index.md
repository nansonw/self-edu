...
```
8 小结：html5(h5 新增特性) 5点
 8.1 统一了文档解析的标准。统一文档类型说明<DOCTYPE !html>
 8.2 新增了语义化标签（多媒体标签）
 8.3 新增了表单类型控件
 8.4 新增了全域属性 contenteditable,draggable,hidden 和特定标签的特定属性 meta-> charset属性
     script -> async属性
 8.5 新增了API。储存（Storage）/ 通讯（Websocket）/计算（Webworker）/定位（Geolocation）/绘图（canvas）
```
### 二 w3c dom
```
1.1 核心DOM:
